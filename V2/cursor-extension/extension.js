...
function activate(context) {
  ...
  // Register command to toggle sound notifications
  let toggleSoundDisposable = vscode.commands.registerCommand(
    "reviewGate.toggleSound",
    () => {
      ...
      const statusMessage = newSetting
        ? "Review Gate sound notifications enabled 🔔"
        : "Review Gate sound notifications disabled 🔇";

      vscode.window.showInformationMessage(statusMessage);

      // Update any open chat panels
      if (chatPanel) {
        chatPanel.webview.postMessage({
          command: "updateSoundSetting",
          enabled: newSetting,
        });
      }
    }
  ...
  context.subscriptions.push(toggleSoundDisposable);
...
function openReviewGatePopup(context, options = {}) {
  ...
  if (triggerId) {
    ...
    console.log(
      `🔍 DEBUG: No triggerId provided, currentTriggerData not updated`
    );
  }
  ...
  chatPanel.webview.onDidReceiveMessage(
    (webviewMessage) => {
      ...
      console.log(
        `🔍 DEBUG: Speech command - currentTriggerData:`,
        currentTriggerData
      );
      console.log(`🔍 DEBUG: Speech command - triggerId:`, triggerId);
      console.log(
        `🔍 DEBUG: Speech command - currentTriggerId:`,
        currentTriggerId
      );
      ...
      switch (webviewMessage.command) {
        ...
        case "showError":
          ...
          break;
        case "toggleSound":
          console.log("🔊 Sound toggle from webview:", webviewMessage.enabled);
...
function getReviewGateHTML(
  title = "Review Gate",
  mcpIntegration = false,
  soundEnabled = true,
  context = null
) {
  // Check for ding.mp3 sound file
  let dingSoundSrc = null;
  if (context) {
    const dingSoundPath = path.join(context.extensionPath, "ding.mp3");
    if (fs.existsSync(dingSoundPath)) {
      // Convert to webview URI
      const dingSoundUri = vscode.Uri.file(dingSoundPath);
      dingSoundSrc = dingSoundUri.toString();
      console.log("🎵 Ding sound file found:", dingSoundPath);
    }
  }
...
<head>
    ...
    <style>
        ...
        .countdown-timer.expired {
            ...
            background: rgba(255, 59, 48, 0.2);
            border-color: rgba(255, 59, 48, 0.5);
            animation: timerExpired 0.5s ease-in-out infinite alternate;
        }
...
<body>
    <div class="review-container">
        <div class="review-header">
            ...
            <div class="sound-toggle" id="soundToggle" title="Click to toggle sound notifications">
                <i class="fas fa-volume-up" id="soundIcon"></i>
            </div>
            <div class="review-author">by Lakshman Turlapati</div>
        </div>
        
        <!-- Ding sound audio element -->
        ${
          dingSoundSrc
            ? `
        <audio id="dingSound" preload="auto" style="display: none;">
            <source src="${dingSoundSrc}" type="audio/mp3">
        </audio>
        `
            : ""
        }
    ...
    <script>
        ...
        let hasDingSound = ${dingSoundSrc ? "true" : "false"};
        ...
        function toggleSound() {
            ...
            const feedbackMessage = soundEnabled ? '🔔 Sound enabled' : '🔇 Sound disabled';
            addMessage(feedbackMessage, 'system', null, true);
        }
        ...
        function stopCountdownTimer() {
            ...
            countdownTimer.classList.remove('warning', 'expired');
        }
        ...
        function expireTimer() {
            ...
            // Add expired message to chat
            addMessage('⏰ Response timer expired (5 minutes). Please respond as soon as possible.', 'system', null, true);
        }
        
        function playDingSound() {
            if (!soundEnabled) {
                console.log('🔇 Sound is disabled - ding sound not played');
                return;
            }
            
            if (!hasDingSound || !dingSound) {
                console.log('🔇 No ding.mp3 file found - sound not available');
                return;
            }
        ...
        function sendMessage() {
            ...
            // Stop countdown timer when message is sent
            stopCountdownTimer();
            ...
            switch (message.command) {
                ...
                case 'newMessage':
                    ...
                    if (message.mcpIntegration) {
                        ...
                        // Start countdown timer for new MCP messages
                        startCountdownTimer();
                    }
                    break;
                case 'focus':
                    messageInput.focus();
                    // Play ding sound when focus is requested (MCP call)
                    playDingSound();
                    // Start countdown timer for MCP integrations
                    if (mcpIntegration) {
                        startCountdownTimer();
                    }
                    break;
                case 'updateMcpStatus':
                    ...
                    // Start timer when MCP becomes active for integrations
                    if (message.active && mcpIntegration) {
                        startCountdownTimer();
    ...
    const pollForResult = setInterval(() => {
      ...
      if (waitTime >= maxWaitTime) {
        ...
        try {
          ...
          console.log(
            `Could not clean up trigger file on timeout: ${e.message}`
          );
        }
        ...
        micTestProcess.on("close", (testCode) => {
          ...
          if (testCode !== 0) {
            ...
            if (testError.includes("Permission denied")) {
              ...
              errorMsg =
                "Microphone is busy - close other recording applications";
            } else if (testError) {
              errorMsg = `Microphone test failed: ${testError.substring(
                0,
                100
              )}`;
            }
